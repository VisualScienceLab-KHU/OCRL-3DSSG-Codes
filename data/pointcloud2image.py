...
def read_pointcloud(scan_id):
    ...
    """
    plydata = trimesh.load(os.path.join('<Your Path>/3RScan/data/3RScan', scan_id, 'labels.instances.annotated.v2.ply'), process=False)
...
def read_scan_info(scan_id, mode='rgb'):
    scan_path = os.path.join("<Your Path>/3RScan/data/3RScan", scan_id)
...
if __name__ == '__main__':
    ...
    fin_all = open(os.path.join(f'<Your Path>/3RScan/data/3RScan/{args.mode}_all_quanlity.txt'), 'a')
    for i in tqdm(selected_scans):
        ...
        save_path = f'<Your Path>/3RScan/data/3RScan/{i}/multi_view'
        os.makedirs(save_path, exist_ok=True)
        # print(f'======= map pointcloud to image =========')
        class_list, class_weight = get_label('<Your Path>/data/3DSSG_subset/classes.txt')
